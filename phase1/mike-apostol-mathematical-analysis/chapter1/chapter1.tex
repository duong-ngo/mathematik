...
\begin{definition}
    A Cauchy sequence $\seq{a_{n}}$ converges to a rational number $a$ if and only if
    \[
...
\begin{proof}
    ...
    For every $n$
    \[
        \abs{a_{n}}\le \max\{ a_{1}, a_{2}, \ldots, a_{N}, 1 + a_{N+1} \}.
    \]
...
\begin{proof}
    ...
    \begin{enumerate}[label={\textbf{Step \arabic*.}},itemindent=1cm]
        \item ${b^{2}_{n}} > 2$.
              ...
              Hence $b^{2}_{n} > 2$.
        \item Prove that $\seq{b_{n}}$ is a decrease sequence.
              ...
              Then $\seq{b_{n}}$ is a decrease sequence.
        \item Prove that $b$ is smaller than all terms of $\seq{b_{n}}$.

              Assume that there exists a natural number $n_{1}$ such that $b_{n_{1}}\le b$. Since $\seq{b_{n}}$ is decrease, then $b_{n} < b$, for every $n > n_{1}$.
...
\begin{theorem}[Sum of two Cauchy sequences]
...
$\seq{c_{n}}$ is called the sum of $\seq{a_{n}}$ and $\seq{b_{n}}$. We denote $\seq{a_{n}} + \seq{b_{n}} = \seq{a_{n} + b_{n}}$.
...
From the above theorem, we deduce that: for any given two Cauchy sequence classes, there exists a Cauchy sequence class of which members are the sum of two Cauchy sequences from the given classes.

We denote the \textit{sum of two classes of Cauchy sequences} $\alpha$ and $\beta$ by $\alpha + \beta$. For example
\[
