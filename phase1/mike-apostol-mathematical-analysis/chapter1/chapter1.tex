...
\begin{definition}
    ...
    \begin{quotation}
        \noindent for every rational numbers $\varepsilon > 0$, there exists a natural number $N$ which is a function of $\varepsilon$ such that for all natural numbers $m, n$ which are greater than $N$, $\left\vert a_{m} - a_{n}\right\vert < \varepsilon$.
    \end{quotation}
...
Informally, given a positive rational numbers $\varepsilon$, then there exists a natural number $N$ such that the differences of any two terms (whose indices are greater than $N$) is smaller than $\varepsilon$.
...
\begin{proof}
    \begin{itemize}
        ...
        \item Transitive.
              ...
              which a given $\varepsilon > 0$, let's choose $N = \max\{ N_{1}, N_{2} \}$, then for all $n > N$
              \[
...
A Cauchy sequence (with rational terms) can converge to a rational number or not.

\begin{definition}
    A Cauchy sequence (with rational terms) $(a_{n})$ converges to a rational number $a$ if and only if
    \[
        (\forall\varepsilon > 0)(\exists N=N(\varepsilon))(\forall n > N)(\abs{a_{n} - a} < \varepsilon).
    \]

    $a$ is called the limit point of $(a_{n})$.
\end{definition}
...
\begin{theorem}
    If a Cauchy sequence (with rational terms) converges, then it has a unique limit point.
\end{theorem}
...
\begin{theorem}
    If the Cauchy sequence (with rational terms) $(a_{n})$ converges to $a$, then every Cauchy sequence (with rational terms) which is equivalent to $(a_{n})$ also converges to $a$.
\end{theorem}
...
\begin{corollary}
    If the Cauchy sequence (with rational terms) $(a_{n})$ does not converge to any rational number, then every Cauchy sequence (with rational terms) which is equivalent to $(a_{n})$ does not converge to any rational number, either.
\end{corollary}
...
\begin{proof}
    ...
    \begin{enumerate}[label={\textbf{Step \arabic*.}},itemindent=1cm]
        ...
        \item Prove that ${b}^{2} = 2$.
              ...
              \[
                  (\forall\varepsilon > 0)\left(\forall n > N(\varepsilon) = \left\lfloor \frac{1}{\varepsilon} - 1\right\rfloor\right)\left( \frac{1}{2^{n}} < \varepsilon \right)
              \]

              So for every $\varepsilon > 0$, choose $N = \left\lfloor\dfrac{1}{\varepsilon} - 1\right\rfloor + 2$, then for all $n > N$, $\abs{b^{2}_{n} - 2} < \dfrac{1}{2^{n}} < \varepsilon$.
    ...
    $b^{2} = 2$ contradicts to the fact that there is no rational number whose square equals $2$.
