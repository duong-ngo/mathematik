...
In this subsection, we aim to define order relation in $\mathscr{C}_{\mathbb{Q}}$ and show that the order relation satisfies the axioms of the real numbers.

We define two relation (a preorder and a strict preorder) on the set of all Cauchy sequences first, and then an order relation on $\mathscr{C}_{\mathbb{Q}}$.
...
\begin{proof}
    \begin{itemize}
        \item Reflexivity.
              ...
              Due to the definition, a Cauchy sequence precedes itself.
        \item Transitivity.
...
\begin{definition}[Strict preorder relation]
...
The following theorem is the key to allow us to define an \textbf{order} relation (yes, order, not preorder) in $\mathscr{C}_{\mathbb{Q}}$.
...
\begin{theorem}\label{theorem:chapter1:equivalent-cauchy-sequences-and-order}
    ...
    \begin{enumerate}[label={(\roman*)}]
        \item If $\seq{a_{n}}\le\seq{b_{n}}$, $\seq{c_{n}}\le\seq{d_{n}}$.
        \item If $\seq{a_{n}} < \seq{b_{m}}$, $\seq{c_{n}} < \seq{d_{n}}$.
    \end{enumerate}
\end{theorem}
...
\begin{proof}
    ...
    \begin{enumerate}[label={\textbf{Step \arabic*.}},itemindent=1cm]
        ...
        \item $\class{\seq{u_{n}}}$ is an upper bound of $S$.
              ...
              So there exists a Cauchy sequence $\seq{t_{n}}$ (which is a member of $\sigma$) such that $\seq{u_{n}} < \seq{t_{n}}$. It follows that
              \[
                  (\exists N)(\forall n > N)(u_{n} < t_{n}).
              \]

              Because of this, $\seq{u_{N}}\le\seq{t_{n}}$.
