...
\begin{proof}
    ...
    \begin{lemma}\label{lemma:chapter1:nonzero-sequence}
        If the Cauchy sequence $\seq{a_{n}}$ is not equivalent to $\seq{0}$, then there exists a natural number $N$ such that for all $n > N$, $a_{n}\ne 0$.
    \end{lemma}
    ...
    \begin{proof}[Proof of Lemma~\ref{lemma:chapter1:nonzero-sequence}]
        ...
        If $a_{n_{0}}\ge\varepsilon_{0}$, then for all $n > N_{0}$
        \[
            a_{n} = (a_{n} - a_{n_{0}}) + a_{n_{0}} > -\varepsilon_{0} + \varepsilon_{0} > 0.
        \]

        Otherwise $a_{n_{0}}\le-\varepsilon_{0}$, then for all $n > N_{0}$
        \[
            a_{n} = (a_{n} - a_{n_{0}}) + a_{n_{0}} < \varepsilon_{0} + (-\varepsilon_{0}) = 0.
        \]

        We have constructed a natural number $N = N_{1}$ such that $\forall n > N$, $a_{n}\ne 0$.
    \end{proof}
...
\begin{theorem}
    The ``$\le$'' relation amongst Cauchy sequences is partially \textbf{preorder} relation.
\end{theorem}
...
The following theorem allows us to define an \textbf{order} relation (yes, order, not preorder) in $\mathfrak{C}_{\mathbb{Q}}$.
