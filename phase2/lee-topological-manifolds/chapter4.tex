...
\begin{proof}
	By Lemma~\ref{lemma:4.80}, there is a neighborhood $V$ of $A$ such that $\overline{V} \subseteq U$. By Urysohn's lemma, there exists a function $f: X \to \closedinterval{0, 1}$ such that $f\vert_{A}\equiv 1$ and $f\vert_{X\smallsetminus V}\equiv 0$. Therefore $f$ doesn't vanish on $\overline{V}$, which means
	\[
		\operatorname{supp}(f) \subseteq \overline{V} \subseteq U
	\]

	because $\operatorname{supp}(f)$ is the smallest closed set containing $f^{-1}(\set{0})$. Hence $f$ is a bump function for $A$ supported in $U$.
\end{proof}
