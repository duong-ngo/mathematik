...
\begin{proof}
	\begin{enumerate}[label={(\alph*)}]
		\item $\emptyset \in \mathscr{T}$ by definition as $\empty$ is an open subset of $X$. $X^{*} \in \mathscr{T}$ by definition as $X^{*}\smallsetminus X^{*} = \emptyset$ is a compact subset of $X$.
		      ...
		      If $J = \emptyset$ then $\bigcup_{\alpha\in A}U_{\alpha} = \bigcup_{\alpha\in I}U_{\alpha} \in \mathscr{T}$. Otherwise, $J\ne \emptyset$ then $X^{*}\smallsetminus U_{\alpha}$ is a compact subset of $X$ for all $\alpha\in J$, hence $X^{*}\smallsetminus U_{\alpha}$ is closed since $X$ is Hausdorff. Moreover
		      \begin{align*}
...
\begin{note}
	One-point compactification of a local Hausdorff space is unique up to a homeomorphism.
\end{note}
...
\begin{proof}
	Denote by $M^{*}$ the one-point compactification of $M$. We will prove that $M^{*}$ is second countable.
