...
\begin{proof}
	...
	Because $X$ is locally connected, $X$ has a basis $\mathscr{B}$ consisting of connected open subsets of $X$. Let $V$ be an open subset of $Y$, then $q^{-1}(V)$ is open because $q$ is continuous. $q^{-1}(V)$ can be written as a union of elements of $\mathscr{B}$, so $V = q(q^{-1}(V))$ can be written as a union of elements of $\set{ q(B) : B\in\mathscr{B} }$, and every $q(B)$ is a connected open subset of $Y$ (because $q$ is an open quotient map), hence $\set{ q(B) : B\in\mathscr{B} }$ is a basis for $Y$, which means $Y$ is locally connected.

	\textbf{$X$ is locally path-connected.}

	Because $X$ is locally connected, $X$ has a basis $\mathscr{B}$ consisting of path-connected open subsets of $X$. Let $V$ be an open subset of $Y$, then $q^{-1}(V)$ is open because $q$ is continuous. $q^{-1}(V)$ can be written as a union of elements of $\mathscr{B}$, so $V = q(q^{-1}(V))$ can be written as a union of elements of $\set{ q(B) : B\in\mathscr{B} }$, and every $q(B)$ is a path-connected open subset of $Y$ (because $q$ is an open quotient map), hence $\set{ q(B) : B\in\mathscr{B} }$ is a basis for $Y$, which means $Y$ is locally path-connected.

	\textbf{$X$ is locally compact.}
