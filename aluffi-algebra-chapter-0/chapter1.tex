...
\begin{proof}
	On $\mathbb{R}$, we define the relation $\sim$ as follows: $a\sim b$ if and only if $e^{2\pi ia} = e^{2\pi ib}$. Because $e^{2\pi ia} = e^{2\pi ib}$ if and only if $b - a\in\mathbb{Z}$, it follows that $a\sim b$ if and only if $b - a\in\mathbb{Z}$. This is identical to the relation defined in Exercise~\ref{exercise:I.1.6}.

	The image of $f$ is the unit circle $\mathbb{S}^{1}$. So the canonical decomposition of $f$ is
	\begin{figure}[htp]
		\centering
		\renewcommand{\thefigure}{I.2.8}
		\centering
...
\begin{exercise}{I.2.9}\label{exercise:I.2.9}
...
\begin{proof}
	...
	\begin{figure}[htp]
		...
		\begin{tikzcd}
			& & A \\
			Z \arrow{r}{\sigma} \arrow[bend left=30]{rru}[above left]{f_{A}} \arrow[bend right=30]{rrd}[below left]{f_{B}} & A\times B \arrow{ru}{\pi_{A}} \arrow{rd}{\pi_{B}} \\
			& & B
		\end{tikzcd}
		\qquad
		\begin{tikzcd}
			& & A \\
			Z \arrow{r}{\tau} \arrow[bend left=30]{rru}[above left]{f_{A}} \arrow[bend right=30]{rrd}[below left]{f_{B}} & B\times A \arrow{ru}{\pi_{A}} \arrow{rd}{\pi_{B}} \\
			& & B
		\end{tikzcd}
	\end{figure}
	\FloatBarrier
