...
\begin{proof}
    ...
    Let \( B_{r}(x), B_{s}(y) \) be two such open balls and they are intersecting, then \( \left\vert r - s \right\vert < \left\vert x - y \right\vert < r + s \). Let \( z \) be a point in \( B_{r}(x) \cap B_{s}(y) \).
...
\begin{exercise}{A.40}
    ...
    \begin{enumerate}[itemsep=0pt,label={(\alph*)}]
        ...
        \item Every {\color{red}{nonempty}} connected subset of \( X \) is contained in a single component of \( X \).
...
\begin{exercise}{A.49}
    Let \( f, g: \left\lbrack 0, \infty \right\lbrack \to \mathbb{R} \) be defined by \( f(x) = \sqrt{x} \) and \( g(x) = x^{2} \). Show that \( f \) is uniformly continuous but not locally or globally Lipschitz continuous, and \( g \) is locally Lipschitz continuous but not uniformly continuous or globally Lipschitz continuous.
\end{exercise}
...
\begin{proof}
    ...
    Assume for the sake of contrary that \( f \) is locally Lipschitz continuous then there exist a neighborhood \( U \) of \( 0 \) and a constant \( C \) such that \( \left\vert \sqrt{x} - \sqrt{y} \right\vert \le C \left\vert x - y \right\vert \) for every \( x, y \in U \), so \( C > 0 \). Therefore, it \( x \ne y \), we have \( \dfrac{1}{\sqrt{x} + \sqrt{y}} \le C \). The open set \( U \) contains an open set \( \left\lbrack 0, r \right\lbrack \subseteq \left\lbrack 0, \infty \right\lbrack \), so \( \dfrac{1}{\sqrt{0} + \sqrt{1/(C^{2} + r^{2})}} > C \), which is a contradiction. Hence \( f \) is not locally Lipschitz continuous, hence not globally Lipschitz continuous.

    For each \( x \in \left\lbrack 0, \infty \right\lbrack \), consider the neighborhood \( \left\lbrack 0, a \right\lbrack \) where \( x < a \). Let \( C = 2a \) then for every \( x, y \in \left\lbrack 0, a \right\lbrack \), one has \( \left\vert x^{2} - y^{2} \right\vert \le C \left\vert x - y \right\vert \). Hence \( g \) is locally Lipschitz continuous.
...
\begin{proof}
    \begin{enumerate}[itemsep=0pt,label={(\alph*)}]
        ...
        \item Here we don't need the continuity of \( F \). The idea was given by Henno Brandsma.
...
\begin{proof}
    ...
    The proof requires more tools like covering maps and its lift properties, please see Lee's Introduction to Topological Manifolds.
