...
\begin{proof}
    \begin{enumerate}[itemsep=0pt,label={(\alph*)}]
        ...
        \item We will prove a more general result: A second-countable locally compact Hausdorff space \( X \) is paracompact.
              ...
              So \( W_{j} \cap W_{j^{\prime}} \ne \varnothing \) only if \( \left\vert j - j^{\prime} \right\vert \le 2 \). Together with each \( \mathcal{U}_{i} \) being finite, we deduce that \( \bigcup_{i \in \mathbb{N}} \mathcal{U}_{i} \) is locally finite. Hence \( X \) is paracompact.
