...
\begin{theorem}{1.46}[Smooth Invariance of the Boundary]
    Suppose \( M \) is a smooth manifold with boundary and \( p \in M \). If there is some smooth chart \( (U, \phi) \) for \( M \) such that \( \phi(U) \subseteq \mathbb{H}^{n} \) and \( \phi(p) \in \partial \mathbb{H}^{n} \), then the same is true for every smooth chart whose domain contains \( p \).
\end{theorem}

\begin{quotation}
    In other words, if \( p \) is a boundary point then any smooth chart \( (U, \phi) \) whose domain contains \( p \) satisfies \( \phi(U) \subseteq \mathbb{H}^{n} \) is open and \( \phi(p) \in \partial \mathbb{H}^{n} \).

    Consequently, this theorem implies: On a smooth manifold with boundary, the set of boundary points and the set of interior points are disjoint.
\end{quotation}

\begin{proof}
    According to the hypothesis, there is a smooth chart \( (U, \phi) \) for \( M \) such that \( \phi(U) \subseteq \mathbb{H}^{n} \) and \( \phi(p) \in \partial \mathbb{H}^{n} \). Suppose on the contrary that there is a smooth interior chart \( (V, \psi) \) whose domain contains \( p \).
    ...
    \begin{align*}
        ...
        \tau^{-1} = \phi \circ \psi^{-1}: \psi(U \cap V) \to \phi(U \cap V)
    \end{align*}
    \endgroup

    are smooth and homeomorphisms. Let \( x_{0} = \phi(p) \) and \( y_{0} = \psi(p) = \tau^{-1}(x_{0}) \).

    \( \phi(U \cap V) \) is an open subset of \( \mathbb{H}^{n} \) and intersects \( \partial \mathbb{H}^{n} \). According to Emile Borel's lemma (see the first paragraph on ``Smooth Structures on Manifolds with Boundary'' in the book.), there exists a smooth function \( \eta: W \to \mathbb{R}^{n} \) defined on a neighborhood \( W \) of \( x_{0} \) in \( \mathbb{R}^{n} \) such that \( \eta \) and \( \tau \) agree on \( W \cap \phi(U \cap V) \).

    \( W \cap \phi(U \cap V) \) is open in \( \phi(U \cap V) \) so
    \[
        \tau(W \cap \phi(U \cap V)) \subseteq_{\text{open}} \psi(U \cap V) \subseteq_{\text{open}} \mathbb{R}^{n}.
    \]

    There exists an open ball \( B \subseteq \mathbb{R}^{n} \) such that \( y_{0} \in B \subseteq \tau(W \cap \phi(U \cap V)) \). One has \( \eta \circ \tau^{-1}\vert_{B} = \tau \circ \tau^{-1}\vert_{B} = \operatorname{Id}_{B} \). According to the chain rule, for every \( y \in B \), \( D \eta(\tau^{-1}(y)) \circ D\tau^{-1}(y) \) is the identity map, which means \( D\tau^{-1}(y) \) is invertible for every \( y \in B \). By Corollary C.36, \( \tau^{-1}\vert_{B} \) maps \( B \) onto an open subset of \( \mathbb{R}^{n} \). Therefore \( x_{0} \) is contained in \( \tau^{-1}(B) \), which is an open set in \( \mathbb{R}^{n} \), which contradicts the assumption that \( \phi(U) \subseteq \mathbb{H}^{n} \) and \( x_{0} \in \partial \mathbb{H}^{n} \).
\end{proof}
...
\begin{proof}
    ...
    If \( i = j \) then \( \psi_{i} \circ \psi_{i}^{-1} \) is the identity map on \( \psi_{i}(U_{i}) \), which is holomorphic.
    ...
    The transition maps are holomorphic, hence the coordinate charts are holomorphically compatible, which means \( \mathbb{CP}^{n} \) is an \( n \)-dimensional complex manifold.

    Thus \( \mathbb{CP}^{n} \) is a compact \(2n\)-dimensional smooth manifold and a compact \(n\)-dimensional complex manifold.
\end{proof}
