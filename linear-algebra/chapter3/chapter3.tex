...
\begin{lemma}\label{chapter3:product-of-transpositions}
    Một xích độ dài $k$ ($k > 1$) có thể viết được dưới dạng tích của không dưới $k-1$ phép thế sơ cấp.
\end{lemma}
...
\begin{proof}[Chứng minh bổ đề~\ref{chapter3:product-of-transpositions}]
    ...
    \end{align*}
    \par Không có phần tử nào cố định đối với xích này.
    \par Sau khi tác động bằng xích trên, vị trí mới của $x_{i}$ được gọi là vị trí đích.
    \par Nếu ta viết xích trên dưới dạng tích của các phép thế sơ cấp và thực hiện chúng lần lượt (phải qua trái) thì một phép sơ cấp có thể tạo ra một trong ba khả năng sau (vì phép thế sơ cấp chỉ tác động lên 2 phần tử):
    \begin{enumerate}[label = Khả năng \arabic*., itemindent=2cm]
        \item Không có thêm phần tử nào ở đúng vị trí đích.
        \item Có thêm đúng 1 phần tử ở đúng vị trí đích.
        \item Có thêm đúng 2 phần tử ở đúng vị trí đích.
    \end{enumerate}
    \par Phép thế sơ cấp cuối cùng trong phân tích trên sẽ tạo ra thêm 2 phần tử ở đúng vị trí đích.
\end{proof}
