...
\begin{proof}
	...
	Let \( W = (X - B) \cap \bigcap^{n}_{1} U_{i} \) then \( W \) is open and a neighborhood of \( x_{0} \). If \( x \in W \) then \( x \in X - B \subset \bigcup^{n}_{1} A_{\lambda_{i}} \), which means \( x \in A_{\lambda_{j}} \) for some \( j \). Moreover, \( x \in W \subset \bigcap^{n}_{1} U_{i} \) so \( x \in U_{j} \). Therefore \( x \in U_{j} \cap A_{j} = V_{j} \), so \( W \subset \bigcup^{n}_{1} V_{i} \).
